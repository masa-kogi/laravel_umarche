                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <div class="md:flex md:items-center mb-2">
                        ...
                        <div class="md:w-3/12 flex justify-around">
                            ...
                            <div>{{ number_format($product['quantity'] * $product['price']) }}<span class="text-sm text-gray-700">円(税込)</span></div>
                        </div>
                        <div class="md:w-2/12">
                            <form method="post" action="{{ route('user.guest.cart.delete', ['item' => $product['product_id']]) }}">
                                @csrf
                                <button>
                                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                    ...
                    <div>
                        <button onclick="location.href='{{ route('user.guest.cart.checkout') }}'" class="flex ml-auto text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded">購入する</button>
                    </div>
                    @else
                    カートに商品が入っていません
                    @endif
